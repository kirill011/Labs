...
namespace MatrixMultiplication
{
    ...
    static class Matrix
    {
        ...
        public static double[,] MatrixMultiply(double[,] matrix1, double[,] matrix2)
        {
            ...
            {
                string wrongMessage = "Invalid matrixs dimension";
                throw new ArgumentException(wrongMessage);
            }
            ...
            for (int row = 0; row < m1rows; row++)
            {
                for (int column = 0; column < m2columns; column++)
                {
                    ...
                    for (int cell = 0; cell < m1columns_m2rows; cell++)
                    {
                        ...
                        {
                            string wrongMessage = "Invalid cell in firstt matrix: ";
                            wrongMessage += cell.ToString() + " " + row.ToString();
                            throw new ArgumentException(wrongMessage);
                        }

                        if (matrix2[column, cell] < 0)
                        {
                            string wrongMessage = "Invalid cell in second matrix: ";
                            wrongMessage += column.ToString() + " " + cell.ToString();
                            throw new ArgumentException(wrongMessage);
                        }
                    }
